...
class DefaultDeviceSecurityRepository(
    ...
    override suspend fun encrypt(rawData: ByteArray, publicKey: ByteArray): SecurityResult {
        ...
        return try {
            ...
            SecurityResult.Success(SecurePayload(encryptedData = encryptedData, signature = signature))
        } catch (e: Exception) {
            encryptedData?.fill(0)
            val error = SecurityError.EncryptionFailed(e)
